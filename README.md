...
Описание:
– идеи для априорных гипотез придумывал с помощью o3. Датасет нереалистичный как по мне (слишком много фрода), поэтому примерно половину гипотез я отсёк сам прежде чем проверять;
– код для агрегаций и визулизаций писал с помощью той же o3, правил github copilot'ом. (о3 часто использует деприкейтед и делитед методы из polars)
– выводы и предложения возможных решений уже добавлял по графикам.